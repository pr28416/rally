...
import { AlignHorizontalJustifyStart, Plus, Send, Copy, Edit, RefreshCw } from "lucide-react";
...
const Chat = () => {
  ...
  const handleSubmit = async (e: React.FormEvent | null, customInput?: string) => {
    ...
    const updatedMessages = editingMessageIndex !== null && editingMessageIndex < messages.length - 1
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center h-full text-center p-6">
      ...
      <h2 className="text-2xl font-semibold text-blue-500 mb-4">Welcome to Rally&apos;s Compliance Assistant!</h2>
  ...
  const editMessage = async (index: number, newContent: string) => {
    ...
    const updatedMessages = [...messages.slice(0, index), { ...messages[index], content: newContent }];
    setMessages(updatedMessages);
        ...
        <main className="flex flex-row h-[95%] mx-auto bg-white rounded-md">
          <div className="w-1/2 flex flex-col">
            <div className="p-4 pb-5 border-b flex items-center">
              <h2 className="text-lg font-semibold text-blue-500">US States Bills on AI Political Ads</h2>
            </div>
          ...
          <div className="w-1/2 flex flex-col">
            <div className="p-4 border-b flex items-center justify-between">
              <div className="flex items-center space-x-2">
                {isRenaming ? (
                  ...
                  <h2 className="text-lg font-semibold truncate max-w-[200px] text-blue-500">{chatName}</h2>
                )}
              </div>
                    ...
                    <Card className="max-w-[80%] relative group">
                      <CardContent className="p-3">
                        {editingMessageIndex === index && message.role === 'user' ? (
                          <form onSubmit={(e) => {
                            e.preventDefault();
                            const input = e.currentTarget.elements.namedItem('editInput') as HTMLInputElement;
            ...
            <div className="border-t p-4">
              ...
              <p className="text-xs text-center mt-2 text-gray-500 font-semibold">
                Open source AI chatbot built with Llama 3.1 70B, TuneStudio with Cerebras, and Vercel AI SDK.
              </p>
            </div>
          </div>
        </main>
