...
export async function POST(request: Request) {
  try {
    ...
    const scriptResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/script/generate`,
      {
        method: "POST",
        headers: {
    ...
    console.log(
      bRollSegments.map((segment: z.infer<typeof AdSegmentSchema>) =>
        segment.b_roll_search_query
      ),
    );

    const fetchBRollVideo = async (
      segment: z.infer<typeof AdSegmentSchema>,
      retryCount = 0,
    ): Promise<string | null> => {
      ...
      try {
        ...
        console.error(
          `Error fetching video for query "${segment.b_roll_search_query}": ${error}. Retrying...`,
        );
    ...
    const videoUrls = await Promise.all(
      bRollSegments.map(async (segment: z.infer<typeof AdSegmentSchema>) => {
        ...
        if (!videoUrl) {
          console.error(
            `Failed to fetch video for query "${segment.b_roll_search_query}" after multiple attempts.`,
          );
        }
        return videoUrl;
      }),
    ...
    const videoFiles = await Promise.all(
      validVideoUrls.map(async (url, index) => {
    ...
    const ffmpegCommand =
      `ffmpeg -f concat -safe 0 -i "${fileListPath}" -c:v libx264 -crf 23 -preset medium -an "${outputFileName}"`;
    ...
    const { data, error } = await supabase.storage
      ...
      .upload(
        `videos/combined_broll_videos_${Date.now()}.mp4`,
        compressedVideoData,
        {
